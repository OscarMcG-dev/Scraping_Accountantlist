...
# These mirror EnrichmentData / DecisionMaker but carry Field descriptions
# that guide the LLM, and use str defaults everywhere so that strict JSON
# Schema mode works (no Optional unions).
# ---------------------------------------------------------------------------
...
class LLMEnrichmentResponse(BaseModel):
    ...
    Strict JSON Schema mode requires every field present and typed.
    """
    ...
    associated_emails: List[str] = Field(
        default_factory=list, description="Other email addresses found on the site"
    )
    associated_mobile_numbers: List[str] = Field(
        default_factory=list, description="Mobile numbers found, each in E.164 format"
    )
    associated_info: str = Field(
        "",
        description=(
            "Supplementary firmographic detail: professional memberships (CAANZ, CPA, NTAA, IPA), "
            "tax agent registration number, accounting software used (Xero, MYOB, QuickBooks, Sage), "
            "add-on tools (Dext, Hubdoc, WorkflowMax), industry niches served"
        ),
    )
    organisational_structure: str = Field(
        "", description="One of: 'solo practice', 'SMB', 'enterprise', 'franchise', or empty string"
    )
    ...
    decision_makers: List[LLMDecisionMaker] = Field(
        default_factory=list,
        description="Senior decision makers (Partner, Director, Principal, etc.). Exclude juniors/receptionists.",
    )
    confidence_score: float = Field(
        0.0, description="0.0-1.0 confidence in the extraction quality"
    )
    out_of_scope: bool = Field(
        False, description="True if the business is NOT an accounting firm"
    )
    out_of_scope_reason: str = Field(
        "", description="Why the firm is out of scope, or empty string"
    )


# ---------------------------------------------------------------------------
# Web search (Phase 2b) — lightweight schema for DM-only search results
# ---------------------------------------------------------------------------
...
class LLMWebSearchPerson(BaseModel):
    ...
    title: str = Field("", description="Job title at the firm (Partner, Director, Principal, etc.) or empty string")
    qualifications: str = Field("", description="Professional qualifications (CA, CPA, BBus, etc.) or empty string")
    email: str = Field("", description="Email address if found, or empty string")
    phone: str = Field("", description="Phone number in E.164 format (+61XXXXXXXXX) if found, or empty string")
    linkedin: str = Field("", description="LinkedIn profile URL (linkedin.com/in/...) or empty string")
    source: str = Field("", description="Where this person was found (e.g. 'LinkedIn', 'CPA Australia directory', 'firm website')")


class LLMWebSearchResponse(BaseModel):
    """Top-level schema for the web search DM discovery call."""
    people: List[LLMWebSearchPerson] = Field(
        default_factory=list,
        description=(
            "Senior people associated with this firm found in search results. "
            "Include Partners, Directors, Principals, Founders, Owners. "
            "Exclude juniors, admin staff, receptionists."
        ),
    )
    ...
    brief: str = Field(
        "",
        description=(
            "One-line factual summary from search results (location, services, size). "
            "Only include if the crawl returned nothing. Otherwise leave empty."
        ),
    )
...
def _make_strict_schema(schema: dict) -> dict:
    """
    Transform a Pydantic-generated JSON Schema into one compatible with
    OpenRouter strict mode: every object gets additionalProperties=false
    and all properties are required.
    """
    schema = copy.deepcopy(schema)

    # Inline $defs references and remove $defs
...
class CompanyRecord(BaseModel):
    ...
    Fields map directly to Attio Company attribute slugs.
    """
    # Core
    ...
    # Dedup metadata (not exported to Attio)
