...
class CSVExporter:
    ...
    def _company_to_dict(self, company: CompanyData) -> dict:
        ...
        result = {
            ...
            "professional_memberships": "; ".join(company.professional_memberships),
            "is_registered_tax_agent": company.is_registered_tax_agent,
        }
        ...
        for i in range(3):
            if i < len(company.decision_makers):
                ...
                result[f"dm_{i+1}_decision_maker_summary"] = dm.decision_maker_summary.replace("\n", " ").strip()
