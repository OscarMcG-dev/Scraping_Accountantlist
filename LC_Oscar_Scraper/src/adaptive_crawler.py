...
class AdaptiveWebsiteCrawler:
    ...
    def __init__(self, settings: Settings):
        ...
        self.link_analyzer = LinkAnalyzer(settings)
    ...
    async def crawl_intelligently(
        ...
        if crawl_strategy == "main_only":
            ...
            try:
                link_analysis = self.link_analyzer.analyze_links(
                    company_url=url,
    ...
    def _basic_link_discovery(self, internal_links: List[Dict[str, str]]) -> List[str]:
        ...
        for link in internal_links[:20]:  # Limit to avoid too many pages
            url = link.get("url", "")
        ...
        try:
            ...
            async with AsyncWebCrawler(config=self.browser_config) as crawler:
                ...
                if not result.success:
                    log_crawl_failure(logger, url, result.error_message if hasattr(result, 'error_message') else 'Unknown error')
                    return None
    ...
    def _combine_markdown(self, main_markdown: str, sub_results: List[Dict[str, Any]]) -> str:
        """
        Combine markdown from all pages.

        Args:
            main_markdown: Main page markdown
            sub_results: Sub-page results
